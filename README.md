...
2. **`worker/` — воркер `komnata1908-admin`** (код есть в этом
   репозитории). Отдельный Cloudflare Worker, который хранит в KV
   расписание бронирования и ссылки хедера/футера, обслуживает
   страницу `/admin`. Исходники — в `worker/src/index.js`.
3. **Воркер `komnata1908-telegram`** — **исходников в этом
   репозитории нет**, это отдельный, уже задеплоенный Cloudflare
   Worker, который фронтенд дёргает напрямую по адресу
   `https://komnata1908-telegram.petr-sobolew.workers.dev`
   (см. `src/utils/bookingApi.js`). Он принимает заявку на бронь и,
   судя по всему, пересылает её в Telegram-бота/чат. Если код от него
   потерян, его придётся восстанавливать с нуля или искать в
   аккаунте Cloudflare, который его хостит — см. раздел ниже.
...
**Важно:** исходный код этого воркера не хранится в данном
репозитории. Известно о нём следующее (по факту использования из
`src/utils/bookingApi.js`):

- URL: `https://komnata1908-telegram.petr-sobolew.workers.dev`
  (Cloudflare Worker, судя по домену `*.workers.dev` — задеплоен без
  собственного домена, под тем же Cloudflare-аккаунтом, что и
  `komnata1908-admin`).
- Принимает `POST` без дополнительных заголовков авторизации, с JSON
  телом `{ name, tel, date, time, guests, utm }`.
- По названию и общему смыслу («приём заявок в Telegram») — форвардит
  эти данные в чат/бота через Telegram Bot API (`sendMessage` в чат
  с администраторами бара). Формат сообщения, токен бота и chat id —
  внутри этого воркера, точное содержимое неизвестно без доступа к
  его исходникам или консоли Cloudflare.

**Если понадобится восстановить или изменить этот воркер:**

1. Проверить список воркеров в консоли Cloudflare
   (аккаунт `petr-sobolew`) — `komnata1908-telegram` должен быть
   виден там со своим кодом, даже если исходников нет в git;
   `wrangler deploy` из его собственной директории (если она
   существует локально где-то ещё) обновит уже задеплоенный код.
2. Если исходников нет нигде — воркер придётся написать заново:
   минимально это HTTP-обработчик `POST`, который принимает тот же
   JSON и делает `fetch` в
   `https://api.telegram.org/bot<TOKEN>/sendMessage` с текстом,
   собранным из полей заявки, используя секреты `BOT_TOKEN` и
   `CHAT_ID` (по аналогии с тем, как `worker/src/index.js` хранит
   свои секреты через `wrangler secret put`).
3. **Настоятельно рекомендуется** в будущем занести исходники этого
   воркера в этот репозиторий (например, в `worker-telegram/`, по
   аналогии со структурой `worker/`), чтобы не зависеть от того, что
   его код существует только в проде.
...
Секреты бэкенда (`ADMIN_TOKEN_SECRET`, `ADMIN_INITIAL_PASSWORD`) и
секреты воркера `komnata1908-telegram` (токен бота и т.п.) хранятся
...
- **Исходников `komnata1908-telegram` нет в репозитории** — см.
  раздел про этот воркер выше. Это самый большой риск потери
  функциональности проекта: если воркер сломается на стороне
  Cloudflare и исходников нигде не останется, форма бронирования
  перестанет отправлять заявки в Telegram (заявки в Google Таблицу
  продолжат приходить, т.к. это независимый путь).
